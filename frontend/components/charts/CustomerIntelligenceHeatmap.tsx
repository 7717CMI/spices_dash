    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">All Distributors</h2>
            <p className="text-sm text-gray-600 mt-1">
              {region} - {industryCategory} ({customers.length} distributors)
            </p>
          </div>
          <button
            onClick={onClose}
...
export function CustomerIntelligenceHeatmap({ title, height = 600 }: CustomerIntelligenceHeatmapProps) {
  ...
  // Get regions and industry categories from generated customer data
  const { regions, endUserSegments } = useMemo(() => {
    // Extract unique regions and segments from the generated customer data
    const uniqueRegions = new Set<string>()
    const uniqueSegments = new Set<string>()
    
    customerData.forEach(item => {
      uniqueRegions.add(item.region)
      uniqueSegments.add(item.endUserSegment)
    })
    
    // Sort regions and segments for consistent display
    const sortedRegions = Array.from(uniqueRegions).sort()
    const sortedSegments = Array.from(uniqueSegments).sort()
    
    return {
      regions: sortedRegions.length > 0 ? sortedRegions : ['North India', 'South India', 'East India', 'West India'],
      endUserSegments: sortedSegments.length > 0 ? sortedSegments : ['Distributors']
    }
  }, [customerData])
  ...
  if (regions.length === 0 || endUserSegments.length === 0 || customerData.length === 0) {
    return (
      <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
        <div className="text-center">
          <p className="text-gray-500">No data to display</p>
          <p className="text-sm text-gray-400 mt-1">
            Distributor intelligence data is being loaded...
          </p>
        </div>
      </div>
    )
    ...
    <div className="w-full">
      ...
      <div className="relative overflow-auto" style={{ maxHeight: height }}>
        ...
        <div className="inline-block min-w-full">
          ...
          <div className="flex">
            <div className="w-32 p-2 bg-gray-100 border border-gray-300 font-medium text-xs text-gray-900">
              Industry Category \ Region
            </div>
            {regions.map(region => (
              <div
                key={region}
                className="w-28 p-2 bg-gray-100 border border-gray-300 text-xs font-medium text-center truncate text-gray-900"
                title={region}
              >
                {region}
              </div>
          ...
          {endUserSegments.map((endUserSegment) => (
            <div key={endUserSegment} className="flex">
              <div className="w-32 p-2 bg-gray-100 border border-gray-300 font-medium text-xs truncate text-gray-900" title={endUserSegment}>
                {endUserSegment}
              </div>
              {regions.map((region) => {
                  ...
                  >
                    <span className={`text-xs font-semibold ${
                      count === 0 
                        ? 'text-gray-500' 
                        : (() => {
                            if (maxCount === 0) return 'text-white'
                            const intensity = (count / maxCount) * 100
                            // Use white text for darker backgrounds (intensity >= 60% where darker colors start)
                    ...
                    <div className={`text-[10px] mt-0.5 ${
                      count === 0 
                        ...
                        : (() => {
                            if (maxCount === 0) return 'text-white opacity-75'
                            const intensity = (count / maxCount) * 100
                            return intensity >= 60 ? 'text-white opacity-75' : 'text-gray-700 opacity-75'
                          })()
                    }`}>
                      distributors
                    </div>
          ...
          >
            ...
            {displayCustomers.length > 0 && (
              <div className="mt-3 pt-3 border-t border-gray-200">
                <p className="text-xs font-medium text-gray-700 mb-2">Sample Distributors:</p>
            ...
            {hasMore && (
              <div className="mt-3 pt-3 border-t border-gray-200">
                ...
                >
                  ...
                  <span>Explore All {hoveredCell.count.toLocaleString()} Distributors</span>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
