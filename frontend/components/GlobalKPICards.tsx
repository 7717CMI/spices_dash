...
export function GlobalKPICards() {
  ...
  const kpiData = useMemo(() => {
    ...
    // Get the global geography from metadata (could be 'Global' or 'India' etc.)
    const globalGeography = data.dimensions.geographies.global?.[0] || 'Global'
    
    // Get all global records from value data
    const globalRecords = data.data.value.geography_segment_matrix.filter(
      record => record.geography === globalGeography
    )
