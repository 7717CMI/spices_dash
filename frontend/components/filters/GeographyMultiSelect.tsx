...
export function GeographyMultiSelect() {
  ...
  const geographyOptions = useMemo(() => {
    if (!data) return { regions: [], countries: {}, global: [] }
    
    const { regions, countries, global } = data.dimensions.geographies
    
    // Filter based on search
    if (!searchTerm) {
      return { regions, countries, global: global || [] }
    }
    
    const search = searchTerm.toLowerCase()
    const filteredCountries: Record<string, string[]> = {}
    const filteredGlobal = (global || []).filter(g => g.toLowerCase().includes(search))
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-96 overflow-hidden">
          ...
          <div className="p-3 border-b">
            <input
              ...
              className="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-gray-500"
            />
          </div>
          ...
          <div className="overflow-y-auto max-h-64">
            {/* Global Option - Dynamic based on data */}
            {geographyOptions.global && geographyOptions.global.length > 0 && geographyOptions.global.map(globalGeo => (
              <label key={globalGeo} className="flex items-center px-3 py-2 hover:bg-blue-50 cursor-pointer border-b">
                <input
                  type="checkbox"
                  checked={filters.geographies.includes(globalGeo)}
                  onChange={() => handleToggle(globalGeo)}
                  className="mr-3 h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
                />
                <span className="text-sm font-semibold text-gray-700">{globalGeo}</span>
                {filters.geographies.includes(globalGeo) && (
