import type { DataRecord, FilterState, ChartDataPoint, HeatmapCell, ComparisonTableRow, ComparisonData, GeographyDimension } from './types'

/**
 * Expand parent geographies to include their children
 * e.g., "India" -> all regions and states, "North India" -> all states in North India
 * 
 * When a parent geography is selected, we want to sum up its children.
 * So we exclude the parent itself and include all its children.
 */
function expandGeographies(
  selectedGeographies: string[],
  geographyHierarchy: GeographyDimension
): string[] {
  ...
  selectedGeographies.forEach(geo => {
    // If "India" is selected, include all regions and all states (exclude "India" itself to avoid double-counting)
    if (geo === 'India' || geographyHierarchy.global.includes(geo)) {
      // Add all regions
      geographyHierarchy.regions.forEach(region => expanded.add(region))
      // Add all states from all regions
      Object.values(geographyHierarchy.countries).forEach(states => {
        states.forEach(state => expanded.add(state))
      })
      // Don't add "India" itself - we want the sum of all regions/states
    }
    // If a region is selected, include all its states (exclude the region itself)
    else if (geographyHierarchy.regions.includes(geo)) {
      const states = geographyHierarchy.countries[geo] || []
      states.forEach(state => expanded.add(state))
      // Don't add the region itself - we want the sum of all its states
    }
    // If a state is selected, include it directly
    else {
      expanded.add(geo)
    }
  })
...
export function filterData(
  ...
  // Expand parent geographies to include children
  const expandedGeographies = geographyHierarchy
    ? expandGeographies(filters.geographies, geographyHierarchy)
    : filters.geographies
  
  const filtered = data.filter((record) => {
    // Geography filter - check if record's geography is in expanded list
    const geoMatch = expandedGeographies.length === 0 ||
