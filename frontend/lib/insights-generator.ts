...
import { DataRecord, FilterState, Metadata } from './types'
...
export function generateInsights(
  ...
  const topPerformer = findTopPerformer(records, filters, metadata)
 ...
 */
function findTopPerformer(records: DataRecord[], filters: FilterState, metadata?: Metadata): Insight | null {
  ...
  // Get currency and unit from metadata, fallback to defaults
  const currency = metadata?.currency || 'USD'
  const unit = filters.dataType === 'value' 
    ? (metadata?.value_unit || 'Mn')
    : (metadata?.volume_unit || 'KT')
  
  return {
    id: 'top-performer',
    type: 'leader',
    title: `${groupKey === 'geography' ? 'Leading Market' : 'Top Segment'}`,
    description: `${topKey} leads with ${topValue.toFixed(1)} ${currency} ${unit} in ${currentYear}`,
    value: topValue,
    trend: 'up',
    priority: 'high',
    icon: '🏆'
