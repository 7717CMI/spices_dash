...
export function D3BubbleChart({ title, height = 500 }: BubbleChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    filtered.forEach(record => {
      const value = record.time_series[currentYearStr] || record.time_series[currentYear] || 0
    ...
    grouped.forEach((records, key) => {
      ...
      records.forEach(record => {
        const startVal = record.time_series[startYearStr] || record.time_series[startYear] || 0
        const endVal = record.time_series[currentYearStr] || record.time_series[currentYear] || 0
        totalValue += endVal
